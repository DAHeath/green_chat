...
void connection_thread::send_room_list() {
  std::vector<uint64_t> rooms;
  rooms.push_back(_c.room_id());
  std::vector<std::string> roomNames;
  roomNames.push_back(_c.room_name());
  auto room_list = _f.build_room_list(rooms, roomNames);
  _c.sendtoOne(room_list.to_string());
}
